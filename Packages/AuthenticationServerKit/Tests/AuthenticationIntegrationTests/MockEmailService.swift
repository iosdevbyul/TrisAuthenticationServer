...
final class MockEmailService: EmailSending, @unchecked Sendable {
    ...
    var sentResetURL: String? { url(subject: "reset") }
    var sentVerificationURL: String? { url(subject: "verify") }
    var sentEmailChangeURL: String? { url(subject: "change") }
    ...
    func send(_ message: EmailMessage, on req: Request) async throws {
        let gate: EmailDeliveryGate? = lock.withLock {
            guard message.subject == "change" else { return nil }
